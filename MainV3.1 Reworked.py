...
def breed_two_chromosomes(first, second):
    if random.uniform(0, 1) <= crossRate:
        ...
        offspring2 = Chromosome(genes=None, code=(second.code[0: crossover] + first.code[crossover:]))
...
def mutate(chromosome):
    ...
    if random.uniform(0, 1) <= mutationRate:
        cellIndex = random.randint(0, len(chromoList))
...
def main():
    ...
    while (not solution_found):
        ...
        for i in range(POOL_SIZE // 2):
            ...
            print("child1: ", childs[0].result)
            print("child2: ", childs[1].result)
            print("")
