...
def main():
    ...
    figure = plt.figure(1)
    fitnessGraph = figure.add_subplot(2,1,2)
    ...
    for i in range(POOL_SIZE):
        valueList.append([])
    targetList = []
    solutionGraph=figure.add_subplot(2,1,1)
    # solutionGraph.set_xlabel("Generation")
    solutionGraph.set_ylabel("Value")
    solutionGraph.set_ylim(bottom=0, top=20, emit=True, auto=False)

    bestList=[]

    bestfitnessList=[]

    # Constant distance from Target or Variable
    ...
    while (not solution_found):
        ...
        variance=(POOL_SIZE/2)*1000
        total_population_fitness = 0
        for i in population:
            total_population_fitness += i.fitness
        avg_fitness = variance*(total_population_fitness / len(population))
        ...
        bestfitnessList.append(bestChromosome.fitness*((POOL_SIZE/2)*1000))
        fitnessGraph.plot(x, bestfitnessList)
        ...
        # bottomLimit=0-TARGET*1
        # topLimit=TARGET*3

        if displayMode=="constant":
            difference = abs(TARGET - bestChromosome.result)
            difference = abs(TARGET - bestChromosome.result) + (difference * 0.75)
        elif displayMode=="variable":
            difference = abs(TARGET - bestChromosome.result) + (TARGET/10)

        bottomLimit = TARGET-difference
        topLimit = TARGET + difference
        solutionGraph.set_ylim(bottom=bottomLimit, top=topLimit, emit=True, auto=False)
        solutionGraph.set_xlim(left=generation-5, right=generation+0.3)
        # print("Bottom Limit: ", bottomLimit)
        # print("Top Limit: ", topLimit)

        # Plot each chromosome value
        for i in range(POOL_SIZE):
            valueList[i].append(population[i].result)
            solutionGraph.plot(x, valueList[i],'ko', lw=0.75+(1.25*(4/(POOL_SIZE*POOL_SIZE))))

        solutionGraph.plot(x, targetList, "o-")
        # solutionGraph.set_xlabel("Generation")
        solutionGraph.set_ylabel("Value")

        bestList.append(bestChromosome.result)
        solutionGraph.plot(x, bestList, "r+-")
